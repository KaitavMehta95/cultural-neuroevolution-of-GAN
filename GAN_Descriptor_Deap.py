...
def mutGAN(individual):
    ...
    if type_mutation=="network_loops":       # The number of loops for the network learning is changed
        ...
        #if(network ==0):
        #    fit_ind.gan_descriptor.Disc_network
        fmeasure = divergence_measures[np.random.randint(len(divergence_measures))]
...
global_belief_generator = []
global_belief_discriminator = []
def beliefSpaceUpdate(fit_individual):
    #keep 5 fittest globals in generator and discriminator as a normative belief space. And infulence the offsprings based on them.
    if(len(global_belief_generator) < 2):

        global_belief_generator.append(copy.deepcopy(fit_individual))
        global_belief_discriminator.append(copy.deepcopy(fit_individual))

    else:
    #Update only generator or discriminator after every generation
        rand = random.randint(0,2)
        if(rand == 0):
            #update generator belief space
            if(len(global_belief_generator) < 5):
                global_belief_generator.append(copy.deepcopy(fit_individual))
            else:
                fitness = fit_individual.fitness.values[0]
                fits = [ind.fitness.values[0] for ind in global_belief_generator]
                # find the least fit in population
                min_fitness_individual = global_belief_generator[fits.index(max(fits))]
                if (fitness <= min_fitness_individual.fitness.values[0]):
                    # remove the least fit and update the global belief space
                    index = global_belief_generator.index(min_fitness_individual)
                    del global_belief_generator[index]
                    individual_gan = copy.deepcopy(fit_individual)
                    global_belief_generator.append(individual_gan)
        else:
            #update discriminator belief space
            if (len(global_belief_discriminator) < 5):
                global_belief_discriminator.append(copy.deepcopy(fit_individual))
            else:
                fitness = fit_individual.fitness.values[0]
                fits = [ind.fitness.values[0] for ind in global_belief_discriminator]
                # find the least fit in discriminator population
                min_fitness_individual = global_belief_discriminator[fits.index(max(fits))]
                if (fitness <= min_fitness_individual.fitness.values[0]):
                    # remove the least fit and update the global belief space
                    index = global_belief_discriminator.index(min_fitness_individual)
                    del global_belief_discriminator[index]
                    individual_gan = copy.deepcopy(fit_individual)
                    global_belief_discriminator.append(individual_gan)
...
def Apply_GA_GAN(toolbox,pop_size=10,gen_number=50,CXPB=0.7, MUTPB=0.3):
    ...
    while g <250:
        ...
        print("Fittest population :",fits.index(min(fits)),"-->",pop[fits.index(min(fits))].fitness.values[0])
        print("_____________________BEST FITNESS : ",pop[fits.index(min(fits))].fitness.values[0],"_____________________")
        print(fits)

        belief_individual = pop[fits.index(min(fits))]
        beliefSpaceUpdate(belief_individual)
        ...
        if(random.randint(1,101) < 30):
                ...
                # Retrieve the fit individuals from belief space and influence the knowledge on offspring.

                # Extracting all the fitnesses of offspring and delete the unfit
                fits = [ind.fitness.values[0] for ind in offspring]
                unfit = fits.index(max(fits))
                del offspring[unfit]

                index = random.randint(0, len(global_belief_generator) - 1)
                fit_ind = global_belief_generator[index]
                index2 = random.randint(0, len(global_belief_discriminator) - 1)
                fit_ind2 = global_belief_discriminator[index2]

                off1 = copy.deepcopy(fit_ind)
                off1.gan_descriptor.Disc_network = copy.deepcopy(fit_ind2.gan_descriptor.Disc_network)
...
if __name__ == "__main__": # Example python3 GAN_Descriptor_Deap.py 0 1000 10 1 30 10 5 50 10 2 0 10 0
    ...
    print("in main function- started executing")
    #   main()      
    ...
    myseed = args.integers[0]               #  111 - Seed: Used to set different outcomes of the stochastic program
    ...
    Function  =  "F"+str(args.integers[3])  # 8-MO function to optimize, all in F1...F9, except F6
    ...
    npop = args.integers[8]                 # 20-Population size
    ngen = args.integers[9]                 # 1-1000-Number of generations
    SEL  = args.integers[10]                # 0-Selection method
    CXp   =  args.integers[11]*0.01         # 20-Crossover probability (Mutation is 1-CXp)
    nselpop = args.integers[12]             #10-Selected population size
    try:
        tournsel_size = args.integers[13]       # 5-Tournament value
    except:
        tournsel_size = 16
