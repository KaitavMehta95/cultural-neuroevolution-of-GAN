...
def init_individual(ind_class):
    ...
    lr = 0.01
    my_gan_descriptor = GAN_Descriptor(fmeasure, netG, gan_no, lr, iter, gloops)
...
#
# Crossover will be backpropogating gradient loss of two different generators.
def cxGAN(ind1, ind2):
    ...
    gen1 = ind1.gan_descriptor
    gen2 = ind2.gan_descriptor
    ind2.gan_descriptor.netG = crossOver(gen1, gen2)
    ind1.gan_descriptor.netG = crossOver(gen2, gen1)
...
class GAN_Descriptor:
    def __init__(self, fmeasure="Standard_Divergence",generator='',gan_no=0,lr=0.001,batch_iteration=1,gen_loop = 1):
...
class GAN:
    def __init__(self, gan_descriptor):
        ...
        elif(self.descriptor.fmeasure == 'CrossEntropyLoss'):
            self.criterion = nn.CrossEntropyLoss()
        elif(self.descriptor.fmeasure == 'L1Loss'):
            self.criterion = nn.L1Loss()
        elif(self.descriptor.fmeasure == 'KLDivLoss'):
    ...
    def training_definition(self):
        ...
        self.Gen_network = self.descriptor.netG
        self.optimizerG = optim.Adam(self.Gen_network.parameters(), lr=self.descriptor.lr, betas=(beta1, 0.999))
...
if __name__ == "__main__":  # Example python3 GAN_Descriptor_Deap.py 0 1000 10 1 30 10 5 50 10 2 0 10 0
    ...
    stroke_dataset = dset.ImageFolder(root='C:/Users/RACHIT/Desktop/Kaitav Project/FaceGenerationGAN/strokeData/',
                               transform=transforms.Compose([
                                   transforms.Grayscale(1),
                                   transforms.ToTensor()
    ...
    dataloader = torch.utils.data.DataLoader( ConcatDataset(dataset,stroke_dataset), batch_size=128,shuffle=True, num_workers=4)
    ...
    nloops = 10  # Maximum number of loops for training a network  (nloops = 5)
    ...
    divergence_measures = ["BCE", "L1Loss", "KLDivLoss", "MSELoss"]
